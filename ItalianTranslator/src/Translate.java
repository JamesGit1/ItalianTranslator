...
public class Translate {
	...
	public String[] translateText(String languageFrom, String searchText ,TextArea translateInput ) {
		long wordCount = 0;
		...
		*/
		...
		translateInput.appendText("Translates to... \n");
		for(int i = 0; i < translatedList.size(); i++) {
			...
			translateInput.appendText((translatedList.get(i) + space ));
		}
	

		long endTime = new Date().getTime();
		if(startTime==endTime) {
			translateInput.appendText("Too fast to record"+ "\n");
		}
		else {
			long totalTime = endTime - startTime;
			double wps = wordCount/totalTime;
			translateInput.appendText( "\n"+"\n " + "WORDS TRANSLATED PER MILLISECOND: " + wps + "words/ms" + "\n"+ "\n");
		...
		if(wordsToAdd.size()!=0) {
			translateInput.appendText("Seems like some words couldn't be translated..." +"\n");
			translateInput.appendText("Looking up translation and adding them to dictionary" +"\n");
		}
		for (int i = 0; i < wordsToAdd.size(); i++) {
			...
			try {
				if (languageFrom.equals("english")) {
					...
					translateInput.appendText("ENGLISH WORD " + wordsToAdd.get(i) + " TRANSLATED TO " + translatedText + "\n");
					...
					tree.saveDictionary(tree.root);
					translateInput.appendText("ITALIAN WORD " + wordsToAdd.get(i) + " TRANSLATED TO " + translatedText + "\n");
...
// pop up window
	...
	public void clearText(String language,String input  ,TextArea text) {
		text.setText("");
		translateText(language,input ,text);			
	}
